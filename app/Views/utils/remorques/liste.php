...
<div class="container-fluid p-0">
  ...
  <div class="card">
    <div class="card-body">
      <h5 class="card-title">Ajouter une remorque</h5>

      <form class="row" action="<?= base_url(session()->root . '/remorques/') ?>" method="post">
        <div class="mb-3 col-sm-6 col-xl-3 col-lg-3">
          <input type="text" class="form-control" required name="immatriculation" aria-describedby="helpId" placeholder="Immatriculation du remorque">
        </div>
        <div class="mb-3 col-sm-6 col-xl-3 col-lg-3">
          <input type="date" class="form-control" name="vt_fin" aria-describedby="helpId" placeholder="Immatriculation du remorque">
          <p class="form-text text-muted">
            Date de fin visite technique ( À laisser vide si elle n'est pas disponible. )
          </p>
        </div>
        <div class="mb-3 col-sm-6 col-xl-3 col-lg-3">
          <input type="date" class="form-control" name="as_fin" aria-describedby="helpId" placeholder="Immatriculation du remorque">
          <p class="form-text text-muted">
  ...
  <div class="card flex-fill">
    <div class="card-header">
      <div class="d-grid gap-3 d-sm-flex justify-content-between align-items-center">
        <h5 class="card-title">Liste de remorques <span class="text-primary">(<?= sizeof($liste) ?>)</span></h5>
        <input type="search" class=" form-control flex-grow-0" style="max-width: 250px;" id="recherche" placeholder="Rechercher un remorque">
      </div>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h3 class="modal-title fs-5" id="groupdeleteLabel">Supprimer remorque</h3>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
              Confirmez-vous la suppression groupée des remorques cochés?
            </div>
            <div class="modal-footer">
              <form id="group" action="<?= base_url(session()->root . '/remorques/supprimer_groupe/') ?>" class="d-flex gap-3" method="post">
                <?= csrf_field() ?>
      ...
      <tbody>
        <?php foreach ($liste as $ligne) : ?>
          ...
          <tr class=" align-items-center">
            <td class="d-none d-xl-table-cell"><input class="form-check-input" form="group" name="liste[]" id="id-<?= $ligne['immatriculation'] ?>" type="checkbox" value="<?= $ligne['id'] ?>"></td>
          ...
          <div class="modal fade" id="Supprimerremorque<?= $ligne['immatriculation'] ?>" tabindex="-1" aria-labelledby="Supprimerremorque<?= $ligne['immatriculation'] ?>Label" aria-hidden="true">
            <div class="modal-dialog">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <p>Supprimer le remorque:</p>
                  <h6 class="text-primary"><?= $ligne['immatriculation'] ?></h6>
                </div>
                <div class="modal-footer">
                  <form action="<?= base_url(session()->root . '/remorques/supprimer/') ?>" class="d-flex gap-3" method="post">
                    <?= csrf_field() ?>
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Non, annuler</button>
